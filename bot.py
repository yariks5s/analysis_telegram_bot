...
async def create_signal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Usage: /create_signal <SYMBOL> <MINUTES> [<IS_WITH_PHOTO>]
    Example: /create_signal BTCUSDT 60 True

    Note: IS_WITH_PHOTO is an optional argunent and is False by default
    """
    args = context.args
    pair = await input_sanity_check_analyzing(True, args, update)
    if (not pair):
        await update.message.reply_text(f"Usage: /create_signal <symbol> [<is_with_photo>], you've sent {len(args)} argument{plural_helper(len(args))}.")
    else:
        try:
