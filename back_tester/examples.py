...
def example_monte_carlo():
    ...
    mean_balance_key = next(
        (
            k
            for k in ["mean_final_balance", "mean_final_equity", "mean_equity"]
            if k in mc_results
        ),
        None,
    )
    lower_ci_key = next(
        (
            k
            for k in ["confidence_interval_lower", "ci_lower", "lower_bound"]
            if k in mc_results
        ),
        None,
    )
    upper_ci_key = next(
        (
            k
            for k in ["confidence_interval_upper", "ci_upper", "upper_bound"]
            if k in mc_results
        ),
        None,
    )
    drawdown_key = next(
        (
            k
            for k in ["mean_max_drawdown", "mean_drawdown", "max_drawdown"]
            if k in mc_results
        ),
        None,
    )
    profitable_key = next(
        (
            k
            for k in ["profitable_simulations_pct", "profitable_pct", "win_rate"]
            if k in mc_results
        ),
        None,
    )
    ...
    if lower_ci_key and upper_ci_key:
        print(
            f"95% confidence interval: [${mc_results[lower_ci_key]:.2f}, ${mc_results[upper_ci_key]:.2f}]"
        )
