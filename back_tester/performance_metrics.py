...
def calculate_performance_metrics(
    ...
    for equity in equity_curve:
        if equity > running_max:
            running_max = equity
        drawdown_pct = (
            (running_max - equity) / running_max * 100 if running_max > 0 else 0
        )
    ...
    if benchmark_returns and len(benchmark_returns) > 0:
        benchmark_return = (
            (benchmark_returns[-1] - benchmark_returns[0]) / benchmark_returns[0] * 100
        )
...
def plot_equity_curve(
    ...
    drawdowns = [
        (
            (running_max[i] - equity_curve[i]) / running_max[i] * 100
            if running_max[i] > 0
            else 0
        )
...
def generate_performance_report(
    ...
    metrics = calculate_performance_metrics(
        trades, initial_balance, final_balance, benchmark_returns
    )
