...
def monte_carlo_simulation(
    ...
    profits = [t.get('profit', t.get('profit_loss', 0)) for t in trades if ('profit' in t or 'profit_loss' in t)]
...
def plot_monte_carlo_results(
    ...
    ax.fill_between(x, lower_bound, upper_bound, color='green', alpha=0.2, label='95% Confidence Interval')
    
    # Add final equity information
    ax.axhline(y=simulation_results['mean_final_equity'], color='red', linestyle='--', 
              label=f"Mean Final: ${simulation_results['mean_final_equity']:.2f}")
    
    # Annotate key metrics
    plt.annotate(f"Mean Final: ${simulation_results['mean_final_equity']:.2f}\n" +
