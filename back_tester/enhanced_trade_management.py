...
def calculate_atr_based_stops(
    df: pd.DataFrame, multiplier: float = 1.5, period: int = 14
) -> pd.Series:
    """
...
def calculate_volatility_based_take_profits(
    *args, **kwargs
    ...
    if len(args) > 0 and isinstance(args[0], pd.DataFrame):
        ...
        atr_values = calculate_atr_based_stops(
            df.iloc[: i + 1], multiplier=1.0, period=atr_period
        )
...
def should_exit_based_on_time(
    entry_index: int, current_index: int, max_trade_duration: int = 24
) -> bool:
    """
