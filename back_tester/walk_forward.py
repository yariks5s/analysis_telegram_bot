...
class WalkForwardAnalysis:
        ...
        **backtest_kwargs
        ...
        for window in windows:
            ...
            window_result = {
                ...
                "train_return_pct": (train_balance - initial_balance) / initial_balance * 100,
                "test_return_pct": (test_balance - current_balance) / current_balance * 100,
        ...
        overall_metrics = {
            ...
            "consistency_score": self._calculate_consistency([r["test_return_pct"] for r in results]),
            "robustness_score": self._calculate_robustness(results),
            "final_balance": results[-1]["test_balance"] if results else initial_balance,
            "total_return_pct": (results[-1]["test_balance"] - initial_balance) / initial_balance * 100 if results else 0,
        }
    ...
    def plot_results(
        ...
        for i in range(len(windows)):
            ...
            consistency = sum(1 for r in curr_test_returns if r > 0) / len(curr_test_returns)
        ...
        axs[1].plot(window_nums, consistency_metric, 'b-', marker='o', label='Consistency')
        axs[1].plot(window_nums, robustness_metric, 'r-', marker='s', label='Robustness')
