...
def detect_market_regime(
    ...
    if len(df) > lookback_period + atr_period:
        ...
        historical_high_close = np.abs(historical_window["High"] - historical_window["Close"].shift())
        historical_low_close = np.abs(historical_window["Low"] - historical_window["Close"].shift())
        
        historical_tr = pd.concat([historical_high_low, historical_high_close, historical_low_close], axis=1).max(axis=1)
    ...
    price_range = (analysis_window["High"].max() - analysis_window["Low"].min()) / analysis_window["Low"].min()
...
def confirm_entry_criteria(
    ...
    if signal_type == "Bullish":
        ...
        if not trend_confirmed:
            return False, f"Bullish signal not confirmed by recent price action ({bullish_count}/5 bullish candles)"
        ...
        if not trend_confirmed:
            return False, f"Bearish signal not confirmed by recent price action ({bearish_count}/5 bearish candles)"
    
    # Volume confirmation
    # Check if volume is increasing
    recent_volume = df["Volume"].iloc[index-5:index] if "Volume" in df.columns else None
    
    if recent_volume is not None:
        avg_recent_volume = recent_volume.mean()
        avg_prior_volume = df["Volume"].iloc[index-10:index-5].mean() if index >= 10 else avg_recent_volume
