...
class StrategyOptimizer:
        ...
        for param_name, param_range in self.param_ranges.items():
            if isinstance(param_range, tuple) and len(param_range) == 2:
                ...
                if isinstance(min_val, int) and isinstance(max_val, int):
                    ...
                    num_steps = min(int(max_combinations ** (1/len(self.param_ranges))), 10)
                    param_grid[param_name] = np.linspace(min_val, max_val, num_steps).tolist()
            else:
    ...
    def _run_parameter_combinations(
        ...
        for i, combination in enumerate(combinations):
            if i > 0 and i % 10 == 0:
                ...
                print(f"Progress: {i}/{len(combinations)} combinations ({i/len(combinations)*100:.1f}%) - ETA: {remaining:.1f}s")
            ...
            try:
                ...
                target_value = metrics.get(self.optimization_target, 0) if metrics else 0
        ...
        # Find best result
        if self.maximize:
            best_result = max(results, key=lambda x: x["target_value"]) if results else None
        else:
            best_result = min(results, key=lambda x: x["target_value"]) if results else None
        ...
        for i, param_name in enumerate(param_names):
            ...
            if all(isinstance(v, (int, float)) for v in param_values):
                try:
                    ...
                    valid_indices = [j for j, v in enumerate(param_values) if v is not None]
                    ...
                    if valid_param_values:
                        ...
                        # Add trend line
                        x_range = np.linspace(min(valid_param_values), max(valid_param_values), 100)
    ...
    def plot_parameter_heatmap(
        ...
        for p1_val, p2_val, target_val in zip(param1_values, param2_values, target_values):
            ...
            if np.isnan(heatmap_data[p2_idx, p1_idx]) or (
                self.maximize and target_val > heatmap_data[p2_idx, p1_idx] or
                not self.maximize and target_val < heatmap_data[p2_idx, p1_idx]
            ):
        ...
        # Plot heatmap
        heatmap = plt.imshow(heatmap_data, interpolation='nearest', aspect='auto', cmap='viridis')
        ...
        # Set tick labels
        plt.xticks(np.arange(len(unique_param1)), 
                  [f"{x:.2f}" if isinstance(x, float) else str(x) for x in unique_param1],
                  rotation=45)
        plt.yticks(np.arange(len(unique_param2)), 
    ...
    if len(param_names) >= 2:
        for i in range(len(param_names)):
            for j in range(i+1, len(param_names)):
                ...
                param1_values = [r["parameters"].get(param1) for r in results["all_results"]]
                param2_values = [r["parameters"].get(param2) for r in results["all_results"]]
                
                if all(isinstance(v, (int, float)) for v in param1_values + param2_values):
                    heatmap_path = os.path.join(output_dir, f"heatmap_{param1}_{param2}.png")
