...
class CorrelationAnalysis:
    ...
    def load_trade_data(self, trades: List[Dict[str, Any]], initial_balance: float) -> None:
        """
        ...
        timestamp_col = "exit_timestamp" if "exit_timestamp" in trade_df.columns else "timestamp"
    ...
    def _calculate_strategy_returns(self, initial_balance: float) -> None:
        ...
        timestamps = [self.trade_data.index[0] - timedelta(days=1)]  # Start one day before first trade
    ...
    def calculate_market_correlations(self) -> pd.DataFrame:
        ...
        for symbol in aligned_market.columns:
            ...
            combined = pd.DataFrame({
                'strategy': aligned_strategy,
                'market': aligned_market[symbol]
            })
            rolling_corr = combined['strategy'].rolling(window=30).corr(combined['market'])
            
            # Calculate upside/downside correlations
            up_market = combined[combined['market'] > 0]
            down_market = combined[combined['market'] < 0]
            
            up_corr = up_market['strategy'].corr(up_market['market']) if len(up_market) > 5 else None
            down_corr = down_market['strategy'].corr(down_market['market']) if len(down_market) > 5 else None
    ...
    def analyze_market_regime_performance(
        ...
        benchmark_symbol = 'BTC' if 'BTC' in self.price_data else list(self.price_data.keys())[0]
        ...
        volatility_regime = volatility.apply(lambda x: 'high_volatility' if x > high_vol_threshold else 'low_volatility')
        ...
        combined_regime['regime'] = combined_regime['trend'] + '_' + combined_regime['volatility']
        ...
        combined_data = pd.DataFrame({
            'regime': aligned_regime['regime'],
            'strategy_return': aligned_strategy
        })
        ...
        for regime in combined_data['regime'].unique():
            regime_returns = combined_data[combined_data['regime'] == regime]['strategy_return']
