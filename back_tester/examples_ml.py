...
def example_ml_enhanced_backtest(ml_enhancer=None):
    ...
    print(f"{'Profit/Loss':<20} ${standard_balance-initial_balance:<14.2f} ${ml_balance-initial_balance:<14.2f}")
    print(f"{'Total Trades':<20} {len(standard_trades):<14} {len(ml_trades):<14}")
    
    # Calculate win rates
    std_win_rate = sum(1 for t in standard_trades if t.get('profit_loss', 0) > 0) / max(1, len(standard_trades)) * 100
    ml_win_rate = sum(1 for t in ml_trades if t.get('profit_loss', 0) > 0) / max(1, len(ml_trades)) * 100
