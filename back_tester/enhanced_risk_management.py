...
def calculate_tighter_stop_loss(
    ...
    # ATR
    high_low = (
        df["High"].iloc[index - atr_period : index]
        - df["Low"].iloc[index - atr_period : index]
    )
    high_close = np.abs(
        df["High"].iloc[index - atr_period : index]
        - df["Close"].iloc[index - atr_period - 1 : index - 1]
    )
    low_close = np.abs(
        df["Low"].iloc[index - atr_period : index]
        - df["Close"].iloc[index - atr_period - 1 : index - 1]
    ...
    recent_volatility = (
        df["Close"].iloc[index - 10 : index].std()
        / df["Close"].iloc[index - 10 : index].mean()
    )
