...
def evaluate_ml_params(
    ...
    try:
        ...
        logger.info(
            f"Parameter types: symbol={type(symbol)}, interval={type(interval)}, "
            f"candles={type(candles)}, initial_balance={type(initial_balance)}, "
            f"risk_percentage={type(risk_percentage)}"
        )
        logger.info(
            f"ML params types: model_type={type(params.get('model_type'))}, "
        ...
        try:
            ...
            if missing_columns:
                logger.error(
                    f"Missing required columns: {missing_columns}. Available columns: {training_data.columns.tolist()}"
                )
                return -9999, None
...
def test_simplified_evaluation(symbol="BTCUSDT", interval="1h", candles=500, initial_balance=10000.0, risk_percentage=1.0):
    ...
    try:
        logger.info("Running backtest with provided parameters")
        final_balance, trades, backtest_metrics = backtester.run_backtest(
            symbol=symbol,
            interval=interval,
            candles=min(200, candles),  # Use smaller subset for testing
            initial_balance=initial_balance,
            risk_percentage=risk_percentage,
...
if __name__ == "__main__":
    ...
    if test_simplified_evaluation(symbol=args.symbol, interval=args.interval, candles=args.candles,
        ...
        try:
            ...
            joblib.dump(
                best_params,
                f"models/best_ml_params_{args.symbol}_{args.interval}_{datetime.now().strftime('%Y%m%d_%H%M')}.joblib",
            )
