...
class MLSignalEnhancer:
    ...
    def _create_feature_selector(self) -> Any:
        ...
        if self.feature_selection == "none":
            ...
            raise ValueError(
                f"Unsupported feature selection method: {self.feature_selection}"
            )
    ...
    def prepare_data_for_training(
        ...
        future_returns = (
            df["close"].pct_change(prediction_horizon).shift(-prediction_horizon)
        )
        ...
        features = df.drop(
            ["open", "high", "low", "close", "volume", target_column], axis=1
        )
    ...
    def train(
        ...
        X, y = self.prepare_data_for_training(
            data, target_column, prediction_horizon, threshold
        )
        ...
        if use_time_series_split:
            ...
            for metric in ["accuracy", "precision", "recall", "f1"]:
                ...
                results[f"{metric}_std"] = np.std(
                    [score[metric] for score in cv_scores]
                )
    ...
    def enhance_signals(
        self, data: pd.DataFrame, signals: pd.Series, threshold: float = 0.6
    ) -> pd.Series:
        """
