...
class TransactionCostsModel:
    ...
    def calculate_entry_costs(
        ...
        slippage_pct = self._calculate_slippage(
            entry_price, amount_to_invest, signal_type, atr, volume_24h
        )
    ...
    def calculate_exit_costs(
        ...
        # Calculate slippage based on model
        slippage_pct = self._calculate_slippage(
            exit_price, position_value, signal_type, atr, volume_24h
        )
...
def apply_transaction_costs_to_backtest(
    ...
    if missing_fields:
        print(
            f"Warning: Cannot apply transaction costs - missing required fields: {missing_fields}"
        )
    ...
    if "entry_signal" not in actual_fields:
        ...
        adjusted_trades["temp_signal"] = (
            "buy"  # Default to 'buy' if no signal is provided
        )
    ...
    for i, trade in adjusted_trades.iterrows():
        ...
        timestamp_field = (
            ...
            else "timestamp" if "timestamp" in trade else None
        )

        if (
            timestamp_field
            and atr_series is not None
            and trade.get(timestamp_field) in atr_series.index
        ):
            atr = atr_series[trade[timestamp_field]]

        if (
            timestamp_field
            and volume_24h_series is not None
            and trade.get(timestamp_field) in volume_24h_series.index
        ):
        ...
        signal_type = (
            trade[actual_fields["entry_signal"]]
            if actual_fields.get("entry_signal")
            else "buy"
        )
        ...
        try:
            ...
            if "exit_price" in actual_fields and trade[actual_fields["exit_price"]] > 0:
                ...
                exit_costs = costs_model.calculate_exit_costs(
                    ...
                    signal_type=(
                        "sell" if signal_type.lower() == "buy" else "buy"
                ...
                original_profit = (
                    trade[actual_fields["profit_loss"]]
                    if "profit_loss" in actual_fields
                    else 0
                )
                adjusted_profit = (
                    exit_costs["actual_amount_received"]
                    - entry_costs["original_amount"]
                )

                # Update trade data
                adjusted_trades.loc[i, actual_fields["entry_price"]] = entry_costs[
                    "actual_entry_price"
                ]
                adjusted_trades.loc[i, actual_fields["exit_price"]] = exit_costs[
                    "actual_exit_price"
                ]
                ...
                adjusted_trades.loc[i, "entry_slippage"] = entry_costs[
                    "slippage_amount"
                ]
                ...
                if "profit_loss" in actual_fields:
                    adjusted_trades.loc[
                        i, "original_" + actual_fields["profit_loss"]
                    ] = original_profit
                    adjusted_trades.loc[i, actual_fields["profit_loss"]] = (
                        adjusted_profit
                    )
                else:
