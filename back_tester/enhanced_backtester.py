...
class EnhancedBacktester:
    ...
    def _fetch_data(self, symbol: str, interval: str, candles: int) -> pd.DataFrame:
        ...
        try:
            ...
            return fetch_candles(
                symbol=symbol, desired_total=candles, interval=interval
            )
    ...
    def run_backtest(
        ...
        **kwargs,
        ...
        if use_ml_signals and not ML_SUPPORT:
            print(
                "Warning: ML signal enhancement requested but ML module is not available."
            )
        ...
        if (
            use_ml_signals
            and self.ml_enhancer
            and self.ml_enhancer.is_trained
            and trades
        ):
            try:
                ...
                for trade in trades:
                    if "timestamp" in trade and "signal" in trade:
                        signals[trade["timestamp"]] = (
                            1 if trade["signal"].lower() == "buy" else -1
                        )
                ...
                enhanced_signals = self.ml_enhancer.enhance_signals(
                    data=data, signals=signals, threshold=ml_threshold
                )
                ...
                print(
                    f"Original buy signals: {original_signals}, Enhanced buy signals: {new_signals}"
                )
                print(
                    f"ML enhancement removed {original_signals - new_signals} false signals"
                )
                ...
                for trade in trades:
                    if (
                        "timestamp" in trade
                        and trade["timestamp"] in enhanced_signals.index
                    ):
                ...
                if len(enhanced_trades) > 0:
                    print(
                        f"Using {len(enhanced_trades)} ML-enhanced trades (filtered from {len(trades)})"
                    )
        ...
        if apply_costs and trades:
            ...
            adjusted_df = apply_transaction_costs_to_backtest(
                trades_df, self.transaction_costs_model
            )
    ...
    def generate_comprehensive_report(
        ...
        **kwargs,
        ...
        summary = {
            ...
            "monte_carlo_confidence_lower": mc_results.get(
                "confidence_interval_lower", 0
            ),
            "monte_carlo_confidence_upper": mc_results.get(
        ...
        **fixed_params,
        ...
        def backtest_wrapper(**params):
            ...
            metrics = calculate_performance_metrics(
                trades, all_params.get("initial_balance", 10000), final_balance
            )
        ...
        optimization_dir = os.path.join(
            self.output_dir, f"optimization_{symbol}_{interval}"
        )
    ...
    def compare_strategies(
        ...
        def backtest_wrapper(**params):
            ...
            # Calculate metrics
            metrics = calculate_performance_metrics(
                trades, params.get("initial_balance", initial_balance), final_balance
            )
        ...
        for name, params in strategies.items():
            strategy_funcs[name] = {"func": backtest_wrapper, "parameters": params}
...
def run_enhanced_backtest(
    ...
    if run_optimization:
        ...
        if optimization_results["best_result"]:
            ...
            print(
                f"Optimal parameters found: {best_params}, Sharpe ratio: {best_value:.2f}"
            )
