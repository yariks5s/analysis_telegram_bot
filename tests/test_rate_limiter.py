...
def test_burst_detection():
    ...
    recent_requests = [
        ts
        for ts in limiter.burst_tracking[user_id]
...
async def test_rate_limit_decorator(mock_update, mock_context):
    ...
    with patch(
        "src.core.rate_limiter.rate_limiter.check_limits",
        side_effect=RateLimitExceeded("Test limit exceeded"),
    ):
        result = await test_handler(mock_update, mock_context)
        ...
        assert (
            "limit exceeded"
            in str(mock_update.message.reply_text.call_args[0][0]).lower()
        )
