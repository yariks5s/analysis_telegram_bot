...
class TestExportFunctionality(unittest.TestCase):
        ...
        self.test_signals = [
            {
                ...
                "market_conditions": json.dumps(
                    {"volume_ratio": 1.2, "volatility": 0.05}
                ),
            },
            {
                "id": 2,
                ...
                "market_conditions": json.dumps(
                    {"volume_ratio": 0.9, "volatility": 0.07}
                ),
            },
        ]
    ...
    def test_generate_export_filename(self):
        ...
        self.assertTrue(
            filename.startswith(f"signal_history_user_{self.user_id}_BTC/USDT_")
        )
    ...
    def test_export_user_signals(self):
        ...
        filepath = export_user_signals(
            self.test_signals, self.user_id, "csv", self.test_export_dir, "BTC/USDT"
        )
        ...
        filepath = export_user_signals(
            self.test_signals, self.user_id, "json", self.test_export_dir
        )
    ...
    def test_empty_signals(self):
        ...
        filepath = export_user_signals([], self.user_id, "csv", self.test_export_dir)
