...
async def command_signal_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await handle_error(
            update, 
            "data_fetch", 
            "Failed to retrieve signal history.",
            exception=e
        )
...
def build_history_keyboard(signals, selected_pair=None):
    ...
    if selected_pair:
        keyboard.append([InlineKeyboardButton("Show All", callback_data=f"{HISTORY_PERIOD}all")])
...
def format_signal_history(signals, max_signals=10):
    ...
    for i, signal in enumerate(signals):
        ...
        line += f"   Entry: {signal['entry_price']:.2f}, SL: {signal['stop_loss']:.2f}, "
        ...
        reasons = json.loads(signal["reasons"]) if isinstance(signal["reasons"], str) else signal["reasons"]
