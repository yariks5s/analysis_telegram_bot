...
def signal_data_to_json(
    signals: List[Dict[str, Any]], filepath: Optional[str] = None
) -> str:
    """
    ...
    for signal in signals:
        ...
        if "market_conditions" in signal_copy and isinstance(
            signal_copy["market_conditions"], str
        ):
            try:
                signal_copy["market_conditions"] = json.loads(
                    signal_copy["market_conditions"]
                )
            except json.JSONDecodeError:
                pass
    ...
    for idx, signal in enumerate(signals):
        ...
        if "market_conditions" in signal:
            if isinstance(signal["market_conditions"], str):
                try:
                    df.at[idx, "market_conditions"] = json.dumps(
                        json.loads(signal["market_conditions"])
                    )
                except json.JSONDecodeError:
...
def generate_export_filename(
    user_id: int, format_type: str, currency_pair: Optional[str] = None
) -> str:
    """
    ...
    return (
        f"signal_history_user_{user_id}{currency_str}_{timestamp}.{format_type.lower()}"
    )


def export_user_signals(
