...
def input_sanity_check_text(text: str) -> dict:
    ...
    if len(parts) < 2:
        result["is_valid"] = False
        result["error_message"] = "Please provide both symbol and period in minutes (e.g., 'BTCUSDT 60')"
        return result
    ...
    if len(parts) >= 3:
        try:
            ...
            if chart_value in ["1", "true", "yes", "y"]:
                ...
                result["is_valid"] = False
                result["error_message"] = "Third parameter should be 0/1, true/false, or yes/no"
                return result
        except ValueError:
            result["is_valid"] = False
...
def check_signal_limit_by_id(user_id: int) -> tuple:
    ...
    if len(previous_signals) >= 10:
        message = f"You've reached the limit of signals ({len(previous_signals)}). "\
                 f"If you want to add a new signal, please remove some of existing signals."
